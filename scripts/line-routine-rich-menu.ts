...
import {
    ...
    type RoutineRichMenuProvisionResult,
    type RoutineRichMenuStatus,
...
function printProvisioningPlan(result: RoutineRichMenuProvisionResult): void {
    console.log("LINE Routine Rich Menu plan");
    console.log(`  LIFF URL: ${result.liffUrl}`);
    ...
    console.log(`  Routine feature: ${result.routineFeatureEnabled ? "enabled" : "disabled"}`);
    console.log("  Area: entire image → Routine LIFF");
}
...
function printStatus(status: RoutineRichMenuStatus): void {
    ...
    console.log(`  Routine feature: ${status.routineFeatureEnabled ? "enabled" : "disabled"}`);
    console.log(`  Expected LIFF URL: ${status.liffUrl ?? "not available"}`);
...
async function runProvision(apply: boolean): Promise<void> {
    const result = await provisionRoutineRichMenu({ apply });
...
async function runStatus(): Promise<void> {
    printStatus(await getRoutineRichMenuStatus());
}
