...
export async function GET(
    ...
    try {
        ...
        if (
            ...
            ticket.reportedById !== parseInt(session.user.id)
        ) {
            return NextResponse.json(
                { error: "Access denied" },
                { status: 403 }
...
export async function PATCH(
    ...
    try {
        ...
        const { status, assignedToId, priority, title, description, category } =
            updateData;
        ...
        if (status && status !== existingTicket.status) {
            try {
                const emailData: TicketEmailData = {
                    ...
                    reportedBy: {
                        name:
                            updatedTicket.reportedBy.employee?.firstName &&
                            updatedTicket.reportedBy.employee?.lastName
                        ...
                        department:
                            updatedTicket.reportedBy.employee?.dept?.name,
                    },
                    assignedTo: updatedTicket.assignedTo
                        ? {
                              name:
                                  updatedTicket.assignedTo.employee
                                      ?.firstName &&
                                  updatedTicket.assignedTo.employee?.lastName
