...
import { reopenRoutineOccurrence } from "@/lib/services/routine";
...
export async function POST(
    ...
    try {
        ...
        const occurrence = await reopenRoutineOccurrence(Number(parsedId.data), parsed.data, actor);
        return NextResponse.json({ occurrence });
    } catch (error) {
        return routineErrorResponse(error, "Error reopening routine occurrence");
    }
}
