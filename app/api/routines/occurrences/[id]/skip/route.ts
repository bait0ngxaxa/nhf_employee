...
import { skipRoutineOccurrence } from "@/lib/services/routine";
import {
...
export async function POST(
    ...
    try {
        ...
        const occurrence = await skipRoutineOccurrence(Number(parsedId.data), parsed.data, actor);
        return NextResponse.json({ occurrence });
    } catch (error) {
        return routineErrorResponse(error, "Error skipping routine occurrence");
    }
}
