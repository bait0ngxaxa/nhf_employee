        ...
        <>
            <div className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] left-[calc(1rem+env(safe-area-inset-left))] right-[calc(1rem+env(safe-area-inset-right))] z-30 sm:left-auto sm:right-[calc(1.5rem+env(safe-area-inset-right))] sm:w-auto">
                <Button
                    ...
                    className="group/cart-bar flex h-auto min-h-16 w-full items-center justify-between gap-4 rounded-2xl border border-action-primary-border bg-action-primary-solid px-4 py-3 text-left text-content-on-brand shadow-lg shadow-action-primary-shadow/20 transition-colors duration-200 hover:border-action-primary-border-strong hover:bg-action-primary-solid-hover sm:min-w-[340px]"
                >
                    <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                            ...
                            <div className="truncate text-xs font-medium leading-5 text-action-primary-on-solid/85">
                                รวม {props.cartCount} ชิ้น กดเพื่อเปิดตะกร้า
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="rounded-full bg-surface-raised px-2.5 py-1 text-xs font-bold tabular-nums leading-5 text-action-primary-foreground shadow-sm">
                            {props.cartCount}
                        </div>
                        <ChevronRight className="h-4 w-4 text-action-primary-on-solid-muted" />
                    </div>
                </Button>
            </div>
