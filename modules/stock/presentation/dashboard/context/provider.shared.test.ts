...
describe("stock provider shared pagination helpers", () => {
    it("recognizes only the canonical stock dashboard pathname", () => {
        expect(isStockDashboardRoute("/dashboard/stock")).toBe(true);
        expect(isStockDashboardRoute("/dashboard")).toBe(false);
    });
    ...
    it("can remove the inventory page query when shared filters reset pages", () => {
        ...
        expect(url.startsWith("/dashboard/stock")).toBe(true);
