...
สำหรับ personal LINE, `LEAVE_ACTION_LINE`, `LEAVE_CANCELLATION_REQUESTED_LINE` และ `LEAVE_NOT_TAKEN_REQUESTED_LINE` ใช้ Leave workflow validator ชุดเดียวกับ available actions/effective approver และ lock `LeaveRequest` ก่อนตรวจ หาก stale ให้ mark child เป็น `SUPERSEDED` ใน transaction แล้วจบโดยไม่เรียก provider; provider call ทำหลัง transaction commit แล้วเท่านั้น
