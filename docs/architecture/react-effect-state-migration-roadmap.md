...
| SSE-038 | `modules/routine/presentation/liff/LiffRoutineTaskDetail.tsx` | 133:9 | D | D — detail-sheet session reset | `deleteConfirmOpen` | `canDelete` and `detail.id` | transient confirmation belongs to current detail session | new detail or changed capability must not retain confirmation | key detail sheet by detail identity or explicit detail-session reset owned by parent | Yes | Low | L5B | `LiffRoutine.test.tsx`; detail switch, close/reopen, capability loss | confirmation is transient; do not reset unrelated detail loading or mutation error state |
