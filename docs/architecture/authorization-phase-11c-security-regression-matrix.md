...
Status: Phase 11C.1 corrective audit pass complete; Phase 11C.2 not started.

Baseline: `main` at `f78b7b35afdae1cd0158d8d9ae5600070c6acb1f`.

Previous matrix implementation baseline: `963b7d76752e4a44b56facd0c463d6d28df6d89f`.
...
breadth. The deferred rows are evaluated against their current deferred
invariants; choosing their future target policy is outside the active test-gap
count. These are test and policy-scope findings, not silently upgraded target
behavior.

The operation-level ledger is finite: it contains 81 migrated
route/capability/channel rows and a separate 17-row Authorization
Administration command ledger. Eight migrated route rows have no adequate
operation-specific direct regression test; all eight are named in `API-10` and
the Phase 11C.2 backlog. This is a coverage finding, not evidence that the
production database or a persisted grant inventory is empty.
...
| Ledger ID | Domain | Entry point / method or command | Capability | Channel | Authentication boundary | Trusted actor provenance | Resource relationship / scope | Transaction revalidation applicability | Existing direct test(s) | Coverage | Gap |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| LEDGER-ROU-01 | Routine | `GET /api/routines/tasks` | `routine.task.read` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current Employee | `CREATED`/`ASSIGNED`/`ALL` per operation and compatibility mode | `N/A` query path | `__tests__/api/routines-tasks.test.ts`; Routine authorization tests | INDIRECT | — |
| LEDGER-ROU-02 | Routine | `POST /api/routines/tasks` | `routine.task.create` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current Employee | `OWN`/`ALL` per adapter policy | Serializable mutation with current actor re-resolution | `__tests__/api/routines-tasks.test.ts`; Routine mutation tests | INDIRECT | — |
...
| LEDGER-ROU-06 | Routine | `GET /api/routines/occurrences` | `routine.occurrence.read` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current Employee | `ASSIGNED`/`ALL` occurrence workload | `N/A` read path | `__tests__/api/routines-occurrences.test.ts` — authenticated employee context | INDIRECT | — |
| LEDGER-ROU-07 | Routine | `GET /api/routines/occurrences?view=tasks` | `routine.task.read` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current Employee | Work-item relationship and exact `NO_APPLICABLE_GRANT -> ALL` bridge | `N/A` read path | `__tests__/api/routines-occurrences.test.ts`; `modules/routine/application/authorization.test.ts` | INDIRECT | — |
...
| LEDGER-ROU-14 | Routine | `GET /api/routines/imports/:batchId` | `routine.import.manage` | `DASHBOARD` | Workforce/Admin session | Server-derived actor/current Employee | `ALL`; batch ownership/validation is service-owned | Import batch read has no transaction-wide re-read claim | Import application tests do not assert this HTTP route | MISSING | `LEDGER-GAP-ROU-IMPORT` |
| LEDGER-ROU-15 | Routine | `GET /api/routines/imports/:batchId/rows` | `routine.import.manage` | `DASHBOARD` | Workforce/Admin session | Server-derived actor/current Employee | `ALL`; batch/row validation is service-owned | Import row read has no transaction-wide re-read claim | Import application tests do not assert this HTTP route | MISSING | `LEDGER-GAP-ROU-IMPORT` |
| LEDGER-ROU-16 | Routine | `PATCH /api/routines/imports/:batchId/rows/:rowId` | `routine.import.manage` | `DASHBOARD` | Workforce/Admin session | Server-derived actor/current Employee | `ALL`; batch/row validation is service-owned | Transaction-time capability recheck in command path; no route race test | Import application tests do not assert this HTTP route | MISSING | `LEDGER-GAP-ROU-IMPORT` |
| LEDGER-ROU-17 | Routine | `POST /api/routines/imports/:batchId/apply` | `routine.import.manage` | `DASHBOARD` | Workforce/Admin session | Server-derived actor/current Employee | `ALL`; batch apply/workflow rules | Transaction-time capability recheck in apply path; no route race test | Import application tests do not assert this HTTP route | MISSING | `LEDGER-GAP-ROU-IMPORT` |
| LEDGER-ROU-18 | Routine | `POST /api/routines/imports/:batchId/cancel` | `routine.import.manage` | `DASHBOARD` | Workforce/Admin session | Server-derived actor/current Employee | `ALL`; batch cancellation/workflow rules | Transaction-time capability recheck in cancel path; no route race test | Import application tests do not assert this HTTP route | MISSING | `LEDGER-GAP-ROU-IMPORT` |
...
| LEDGER-STK-01 | Stock | `GET /api/stock/categories` | `stock.catalog.read` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current role | Catalog `ALL` | `N/A` read path | Stock authorization/query tests; no dedicated Dashboard category route assertion | INDIRECT | — |
| LEDGER-STK-02 | Stock | `POST /api/stock/categories` | `stock.inventory.manage` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current role | Inventory `ALL` | Serializable inventory mutation path | Stock authorization/mutation tests; no dedicated category route assertion | INDIRECT | — |
| LEDGER-STK-03 | Stock | `DELETE /api/stock/categories` | `stock.inventory.manage` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current role | Inventory `ALL` | Serializable inventory mutation path | Stock authorization/mutation tests; no dedicated category route assertion | INDIRECT | — |
| LEDGER-STK-04 | Stock | `GET /api/stock/items` | `stock.catalog.read` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current role | Catalog `ALL` | `N/A` read path | `modules/stock/__tests__/queries.test.ts`; no dedicated Dashboard item-list auth assertion | INDIRECT | — |
| LEDGER-STK-05 | Stock | `POST /api/stock/items` | `stock.inventory.manage` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current role | Inventory `ALL` | Serializable inventory mutation path | Stock authorization/mutation tests; item route suite focuses update/delete | INDIRECT | — |
...
| LEDGER-STK-08 | Stock | `POST /api/stock/items/:id/adjust` | `stock.inventory.manage` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current role | Target inventory item `ALL` plus quantity invariants | Serializable inventory mutation path | `modules/stock/__tests__/mutations.test.ts`; no dedicated adjust route assertion | INDIRECT | — |
...
`DIRECT`, 12 `INDIRECT`, and 8 `MISSING`. The eight missing rows are
`LEDGER-EMP-04`, `LEDGER-ROU-08`, `LEDGER-ROU-14` through
`LEDGER-ROU-18`, and `LEDGER-LEV-17`; they are the finite route-level scope
of `API-10`. Ledger counts are operation evidence and are not added to the 89
matrix-case total.

The optional `stock.request.process` and `stock.request.cancel` rows attached
to LIFF `GET` detail/processing responses describe action-availability
projection checks only. The corresponding `POST` issue/cancel routes are the
mutation authority and are listed separately.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AUTHN-01 | Auth and selected migrated Dashboard/API entry points | Employee list/stats/mutation, Department, Audit, Notification, and Authorization Administration boundaries represented by the named suites | `DASHBOARD` | Missing, invalid, or unauthenticated session must not reach the covered protected work | Return safe `401` and do not perform the covered protected query or mutation | `lib/auth/api.ts:requireApiSession`; route guards before services | `__tests__/api/employees-routes.test.ts`; `__tests__/api/departments-route.test.ts`; `__tests__/api/audit-log-route.test.ts`; `__tests__/api/notifications.test.ts`; `__tests__/api/authorization-administration-mutations.test.ts` | INDIRECT | MIGRATED_AUTHORIZATION | The finite ledger identifies the migrated route rows outside these direct assertions; complete only those named rows in `API-10` | This row does not claim every migrated route is directly tested. UI visibility is not part of this proof. |
...
| ACTOR-02 | API body, query, and headers | The Employee, Authorization Administration, Routine LIFF, Stock LIFF, and Leave LIFF cases named in the test column | `DASHBOARD` or `LIFF_SELF_SERVICE` | Request fields cannot supply actor user ID, role, capability, scope, Team origin, or channel in the covered cases | Authorization result remains based on the server-derived actor and operation contract | Route handlers pass authenticated identity to adapters; central resolver accepts `AuthorizationActor`, not `Request` | `__tests__/api/employees-routes.test.ts`; `__tests__/api/authorization-administration-mutations.test.ts`; `__tests__/api/line-routine-routes.test.ts`; `__tests__/api/line-routine-self-service-routes.test.ts`; `__tests__/api/line-stock-routes.test.ts`; `__tests__/api/line-leave-routes.test.ts` | INDIRECT | MIGRATED_AUTHORIZATION | The ledger names migrated operations without an exact spoofing assertion; add only those finite cases in 11C.2 | Headers are transport metadata only where the route explicitly derives a fixed channel. This row does not generalize the named tests to every migrated operation. |
| ACTOR-03 | Dynamic route parameters | The Admin, Routine occurrence/task, and Stock request target cases named in the test column | `DASHBOARD` | Route IDs identify a target only and cannot supply an authorization role or capability in the covered cases | Use the authenticated actor with the route ID as resource input; reject unauthorized target access | Route handlers and service command contexts; Admin route auth | `__tests__/api/authorization-administration-mutations.test.ts`; `__tests__/api/routines-occurrence-by-id.test.ts`; `__tests__/api/routines-task-by-id.test.ts`; `__tests__/api/stock-requests-routes.test.ts` | INDIRECT | MIGRATED_AUTHORIZATION | The ledger identifies additional dynamic migrated targets requiring exact assertions in 11C.2 | A target ID is not actor provenance; this row does not claim every dynamic route is directly tested. |
...
| SCOPE-09 | Query-level relationship enforcement | Employee, Routine, Stock, Leave, Notification | `DASHBOARD` / `LIFF_SELF_SERVICE` | Unauthorized resources must not be returned before a later UI check in the covered reader cases | Apply actor/resource predicates before the query or return a safe denial | Domain query builders and route authorization gates | `__tests__/api/departments-route.test.ts`; `__tests__/api/notifications.test.ts`; `__tests__/api/stock-requests-routes.test.ts`; `__tests__/api/line-stock-routes.test.ts`; `__tests__/api/leave-me.test.ts`; `__tests__/api/routines-task-by-id.test.ts` | INDIRECT | DOMAIN_OR_LIFECYCLE_POLICY | The operation ledger identifies reader rows without an exact query-before-return assertion; add only those finite cases in 11C.2 | UI filtering is not a substitute for query scoping. This row does not claim every migrated reader is directly tested. |
...
| API-01 | Employee API | Employee route/capability cases with named direct tests | `DASHBOARD` | Direct HTTP invocation must enforce authentication, capability, target relationship, lifecycle, and business rules in the covered cases | Unauthorized calls fail before service mutation/query; authorized calls still pass domain invariants | `app/api/employees/route.ts`; `app/api/employees/[id]/route.ts`; Employee authorization/service/transaction layer | `__tests__/api/employees-routes.test.ts`; `modules/employee/application/authorization.test.ts`; `modules/employee/application/mutations.test.ts` | INDIRECT | MIGRATED_AUTHORIZATION | The finite ledger records Employee export without an operation-specific route assertion; keep compatibility policy separate from target-policy claims | Broad Employee read/stat/export behavior is compatibility policy, not evidence of narrower target policy. |
...
| API-09 | Direct API versus presentation | Migrated protected actions represented by the named route denial suites | `DASHBOARD` / `LIFF_SELF_SERVICE` | A hidden UI control or absent navigation must not be the only protection in the covered cases | Direct HTTP call still hits server auth, authorization, relationship, lifecycle, and domain/workflow checks | Route handlers, service command boundaries, and transaction adapters | Employee, Routine, Stock, Leave, Notification, Admin, and LIFF route denial tests | INDIRECT | MIGRATED_AUTHORIZATION | The finite ledger identifies migrated operation rows without exact direct route proof; do not treat presentation tests as substitutes | Presentation rows cannot be counted as API enforcement. This row does not claim every migrated action is directly tested. |
| API-10 | Migrated route breadth | Every migrated capability and entry point listed in the finite ledger | `DASHBOARD` / `LIFF_SELF_SERVICE` | Each migrated route/capability pair should have a direct regression assertion for its own boundary | The ledger identifies the exact route, actor, capability, relationship, lifecycle, and transaction contract rather than relying on adjacent suites | Source inventory is broad; the operation-level ledger is explicit and test inventory is uneven by operation and channel | The ledger names eight migrated operation rows without adequate direct route proof | MISSING | MIGRATED_AUTHORIZATION | `11C2-API-01`: add direct behavior tests for the eight named ledger rows; do not create an unbounded route-breadth task | This is a coverage gap, not evidence of a production bypass. |
| TX-01 | Mutation-sensitive migrated operations | Authorization state visible to a supported transaction-time re-read | `DASHBOARD` / `LIFF_SELF_SERVICE` | A supported revalidation path must not make its final decision from a stale preflight actor/resource state when a revocation or lifecycle change is visible to that transaction | Final authorization uses the state observed by the path's own lock/re-read boundary; no atomic guarantee is claimed for a grant, membership, or role row committed concurrently in an unrelated transaction after the resolver read | `resolveInTransaction`; `runSerializableTransaction`; domain adapters lock/re-read User, Employee, target Employee, or relationship rows where documented. Grant/membership rows are not generally locked by the resolver | No direct deterministic test changes a supported transaction's visible current state between its initial snapshot and final decision and asserts denial | MISSING | MIGRATED_AUTHORIZATION | `11C2-TX-01`: add only supported transaction-context/lock-re-read cases, and state the isolation/visibility assumption in each test | Distinguish a fresh next-request resolution from in-transaction revalidation. Do not require atomic coordination with an external commit, and do not extend this row to read-only, Employee import, or filesystem paths. |
...
| TX-06 | Cross-domain stale actor state | Current persisted system role in final mutation decision for adapters that claim actor revalidation | `DASHBOARD` / `LIFF_SELF_SERVICE` | A supported mutation adapter must not reuse a stale route-time or client-supplied system role after the current persisted role has changed | Re-read the current User/Employee actor state at the adapter's documented boundary; no claim is made that the resolver atomically observes an unrelated concurrent grant-table commit | Employee explicitly rebuilds the actor; Stock, Routine, and Leave have path-specific current-state checks, but there is no complete direct cross-domain proof | Employee tests are direct; Stock/Routine/Leave tests cover adjacent lifecycle and re-resolution cases, not a paired stale-role proof | MISSING | DOMAIN_OR_LIFECYCLE_POLICY | `11C2-TX-02`: add deterministic current-role replacement cases for the exact Stock, Routine, and Leave transaction adapters; test grant/membership re-resolution separately only where the path claims it | Keep unsupported grant-row race cases classified rather than inventing a guarantee. A fresh next request and a same-transaction re-read are different guarantees. |
...
| `11C2-API-01` | `API-10` | Direct operation-specific route proof is absent for the finite ledger rows `LEDGER-EMP-04`, `LEDGER-ROU-08`, `LEDGER-ROU-14` through `LEDGER-ROU-18`, and `LEDGER-LEV-17` | Add only the eight named route/capability/channel boundary tests; do not create an unbounded “remaining routes” task. |
...
part of `11C2-CAP-02` below. The same distinction applies to the indirect
aggregate rows: an adjacent test is recorded as indirect evidence, not as a
...
| 6 | `11C2-API-01` | Eight exact missing route rows: `LEDGER-EMP-04`, `LEDGER-ROU-08`, `LEDGER-ROU-14` through `LEDGER-ROU-18`, and `LEDGER-LEV-17` | No adequate operation-specific direct route assertion exists for those capability/channel pairs | Add eight finite behavior-oriented route tests covering authn, trusted actor, capability/resource boundary, and applicable lifecycle/workflow behavior. |
...
- `11C2-CHANNEL-01`: pair Dashboard and LIFF assertions for each capability
  intentionally available in both channels (`CHANNEL-07` is currently
  `INDIRECT`).
...
- Add an explicit current-classification assertion that Leave report/export
  ...
  (`COMPAT-10` is currently `INDIRECT`); this is not a future policy decision.
...
This section records the exact commands and observed results after the
corrective matrix pass. The full-suite result remains distinct from focused
authorization results, including the known default timeout limitation.
...
| `npm.cmd run test:run -- modules/authorization __tests__/auth modules/employee/application/authorization.test.ts modules/routine/application/authorization.test.ts modules/stock/application/authorization.test.ts modules/leave/application/authorization.test.ts` | PASS, exit 0. Vitest reported 27 test files and 250 tests passed in 10.78s. |
| `npm.cmd run test:run` | **FAIL**, exit 1. Vitest reported 313 test files passed and 1 failed; 2,758 tests passed and 1 failed out of 2,759. `__tests__/architecture/check-architecture.test.ts` — `reads changed runtime imports again on a later scan of the same root` timed out at 5,000 ms after observing 5,332 ms. Duration 286.90s. |
| `git diff --check` | PASS, exit 0. No whitespace errors reported after the final documentation edit. |
...
scan of the same root` exceeding the default 5000 ms timeout. The same failure
reproduced in this corrective pass. The test, timeout, and configuration were
not changed, and the result is recorded as a runner limitation rather than
rewritten as a pass or attributed to this documentation-only change.

The full-suite result above was observed before the last documentation-only
metric/wording adjustment. No production, test, or configuration file changed;
the architecture, lint, typecheck, and diff checks were rerun after that final
adjustment.
