# Authorization Phase 11C — Security Regression Matrix

Status: **CLOSED — Phase 11C security regression audit and implementation backlog complete.**
...
- `5669d79ca359701bc6a637079ca738575b731cf7` — final/current Phase 11C.2D
  matrix reconciliation and closure audit baseline.

Audit point: final/current production source and tests at the Phase 11C.2D
closure audit baseline `5669d79ca359701bc6a637079ca738575b731cf7`.
Final closure record: [authorization-phase-11c-closure.md](authorization-phase-11c-closure.md)
...
the tests intentionally prove, and the final disposition of the Phase 11C.2
...
The final 89-row reconciliation is `80` `DIRECT`, six legitimate `INDIRECT`,
zero `MISSING`, and three legitimate `N/A`. The six `INDIRECT` rows are
`AUTHN-01` (selected-route authentication aggregate), `CAP-12` (additive model
evidence with source/schema absence inspection), `CHANNEL-07` (separate rather
than exhaustive paired channel evidence), `ADMIN-08` (aggregate current-role
evidence), `COMPAT-10` (deferred Leave report/export classification), and
`COMPAT-15` (future Team architecture boundary). The three `N/A` rows are
`SCOPE-05`, `TX-09`, and `TX-10`: each intentionally declines to claim a
stronger invariant in the current architecture. `INDIRECT` is not a security
defect, and `N/A` does not mean that the route is unprotected.

Zero `MISSING` means that no mandatory Phase 11C regression gap remains in the
claimed matrix and operation scope; it does not mean that every conceivable
authorization test exists.
...
| Mandatory Phase 11C.2 regression backlog | 0 |
...
This final audit did not query or inventory the production authorization database.
...
The code-owned authorization seed configuration is empty at this baseline.
The Phase 11C.2D audit did not query or inventory the production authorization
database; therefore the presence or absence of persisted Team, TeamRole,
membership, or User grant records in production is not asserted by this audit.
...
| --- | --- | --- | --- |
| `LEDGER-ROU-01` | The route builds a server actor and delegates to `getRoutineTasks()`, which resolves `routine.task.read` in the query layer. | The new exact route test executes the route and real query authorization, then proves an unrelated creator-scoped task is not returned. | `DIRECT` — `__tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts` (`LEDGER-ROU-01 denies unrelated task workload through GET /api/routines/tasks`). |
...
| LEDGER-ROU-01 | Routine | `GET /api/routines/tasks` | `routine.task.read` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current Employee | `CREATED`/`ASSIGNED`/`ALL` per operation and compatibility mode | `N/A` query path | `__tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts` — exact route, real query authorization, and creator scope | DIRECT | — |
| LEDGER-ROU-02 | Routine | `POST /api/routines/tasks` | `routine.task.create` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current Employee | `OWN`/`ALL` per adapter policy | Serializable mutation with current actor re-resolution | `__tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts` — exact route, real transaction authorization, idempotency, and no-create resource denial | DIRECT | — |
...
| LEDGER-ROU-06 | Routine | `GET /api/routines/occurrences` | `routine.occurrence.read` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current Employee | `ASSIGNED`/`ALL` occurrence workload | `N/A` read path | `__tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts` — exact route, real query authorization, and server-derived assignee scope | DIRECT | — |
| LEDGER-ROU-07 | Routine | `GET /api/routines/occurrences?view=tasks` | `routine.task.read` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current Employee | Work-item relationship and exact `NO_APPLICABLE_GRANT -> ALL` bridge | `N/A` read path | `__tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts` — exact `view=tasks` capability selection and frozen bridge behavior | DIRECT | — |
| LEDGER-ROU-08 | Routine | `GET /api/routines/occurrences/:id` | `routine.occurrence.read` | `DASHBOARD` | Dashboard workforce/Admin session | Server-derived actor/current Employee | `ASSIGNED`/`ALL` occurrence relationship | `N/A` read path | `__tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts` — exact GET detail route with unrelated-actor resource denial | DIRECT | — |
...
| LEDGER-ROU-14 | Routine | `GET /api/routines/imports/:batchId` | `routine.import.manage` | `DASHBOARD` | Workforce/Admin session | Server-derived actor/current Employee | `ALL`; batch ownership/validation is service-owned | Import batch read has no transaction-wide re-read claim | `__tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts` — exact batch GET denial before service | DIRECT | — |
| LEDGER-ROU-15 | Routine | `GET /api/routines/imports/:batchId/rows` | `routine.import.manage` | `DASHBOARD` | Workforce/Admin session | Server-derived actor/current Employee | `ALL`; batch/row validation is service-owned | Import row read has no transaction-wide re-read claim | `__tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts` — exact rows GET denial before service | DIRECT | — |
| LEDGER-ROU-16 | Routine | `PATCH /api/routines/imports/:batchId/rows/:rowId` | `routine.import.manage` | `DASHBOARD` | Workforce/Admin session | Server-derived actor/current Employee | `ALL`; batch/row validation is service-owned | Transaction-time capability recheck in command path; no route race test | `__tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts` — exact row PATCH denial before mutation; transaction recheck remains in import command tests | DIRECT | — |
| LEDGER-ROU-17 | Routine | `POST /api/routines/imports/:batchId/apply` | `routine.import.manage` | `DASHBOARD` | Workforce/Admin session | Server-derived actor/current Employee | `ALL`; batch apply/workflow rules | Transaction-time capability recheck in apply path; no route race test | `__tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts` — exact apply POST denial before command; transaction recheck remains in import command tests | DIRECT | — |
| LEDGER-ROU-18 | Routine | `POST /api/routines/imports/:batchId/cancel` | `routine.import.manage` | `DASHBOARD` | Workforce/Admin session | Server-derived actor/current Employee | `ALL`; batch cancellation/workflow rules | Transaction-time capability recheck in cancel path; no route race test | `__tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts` — exact cancel POST denial before command; transaction recheck remains in import command tests | DIRECT | — |
...
`DIRECT`, zero `INDIRECT`, and zero `MISSING`. Together with the separate
17-row Administration command ledger, the combined arithmetic is `81 + 17 =
98`; these ledger counts are operation evidence and are not added to the 89
matrix-case total. Excluding the three
action-availability projection rows, the 78 protected route/capability rows
contain 78 `DIRECT` and zero `MISSING` rows.

The three Stock LIFF projection rows attached to `GET` detail/processing
...
| CHANNEL-01 | Dashboard migrated domains | Employee, Department, Routine, Stock, Leave, Audit, Notification | `DASHBOARD` | Dashboard operations use the Dashboard channel and current server actor | Resolve the registered capability for `DASHBOARD`, then apply domain policy | Domain authorization adapters and Dashboard route handlers | `modules/employee/application/authorization.test.ts`; `modules/department/application/authorization.test.ts`; `modules/audit/application/authorization.test.ts`; `modules/notification/application/authorization.test.ts`; `modules/stock/application/authorization.test.ts`; `modules/routine/application/authorization.test.ts`; `modules/leave/application/authorization.test.ts` and corresponding API suites | DIRECT | MIGRATED_AUTHORIZATION | None for covered Dashboard adapters | Broad compatibility floors are separate from central target policy. |
...
| CHANNEL-07 | Cross-channel model parity | Same capability model with explicit channel constraints | `DASHBOARD` / `LIFF_SELF_SERVICE` | Dashboard and LIFF must share registry/resolver semantics without sharing unrestricted channel authority | Same central decision model; channel-specific adapters constrain operation/resource | Domain adapters use the same resolver API with fixed channels | `modules/routine/application/authorization.test.ts`; `modules/stock/application/authorization.test.ts`; `modules/leave/application/authorization.test.ts`; `__tests__/api/line-routine-routes.test.ts`; `__tests__/api/line-stock-routes.test.ts`; `__tests__/api/line-leave-routes.test.ts` | INDIRECT | MIGRATED_AUTHORIZATION | No mandatory Phase 11C.2 gap; `11C2-CHANNEL-01` is OPTIONAL strengthening for paired assertions for each capability intentionally available in both channels | Existing tests establish the two paths separately, not one exhaustive parity table; this aggregate remains INDIRECT. |
...
| TX-08 | Stock upload boundary | `stock.inventory.manage` plus image/file write | `DASHBOARD` | Stale authority must not reach a file-system write after preflight | Require capability before parsing/file work and immediately before the write | `app/api/uploads/image/route.ts`; Stock authorization adapter | `__tests__/api/uploads-image-route.test.ts` | DIRECT | DOMAIN_OR_LIFECYCLE_POLICY | No mandatory Phase 11C gap; documented residual file-system TOCTOU remains accepted and may be addressed only if atomic file authorization is later required | Database authorization and file-system write are not one atomic transaction. |
| TX-09 | Read-only and explicitly non-transactional paths | Reads, Employee create/import, and other paths without a transaction-time claim | `DASHBOARD` / `LIFF_SELF_SERVICE` | Do not invent transaction guarantees that the current path does not claim | Apply route preflight and the documented domain/lifecycle policy; classify stronger guarantees as future work | Per-route authorization and service boundaries; no `resolveInTransaction` claim for these paths | `__tests__/api/employees-routes.test.ts`; `__tests__/api/routine-import-preview.test.ts`; read-route authorization tests named in the operation ledger | N/A | DOMAIN_OR_LIFECYCLE_POLICY | No mandatory Phase 11C gap: the stronger transaction-wide invariant is intentionally not claimed; require an explicit policy decision before adding a guarantee | N/A means the transaction-time invariant is not claimed, not that the route is unauthenticated or unprotected. |
| TX-10 | Routine import | `routine.import.manage` | `DASHBOARD` | Partial import semantics do not claim a transaction-wide authorization re-read | Keep the documented preflight/parse/validation behavior; do not label it transactionally revalidated | Routine import route and service | `__tests__/api/routine-import-preview.test.ts`; Routine import application tests | N/A | DOMAIN_OR_LIFECYCLE_POLICY | No mandatory Phase 11C gap: the stronger transaction-wide re-read is intentionally not claimed; defer any stronger semantics to a separately scoped Routine decision | This surface is migrated for capability gating but has no transaction-wide re-read claim; N/A does not mean that import authorization is absent. |
...
| ADMIN-04 | Domain and workflow invariants | All ADMIN mutations | `DASHBOARD` / `LIFF_SELF_SERVICE` | ADMIN cannot bypass invalid state transitions, resource relationships, business rules, or validation | Domain command remains subject to current state, ownership/assignment semantics, and validation | Domain services and transaction adapters after authorization | `modules/employee/application/mutations.test.ts`; `modules/routine/application/mutations.test.ts`; `modules/stock/__tests__/mutations.test.ts`; `__tests__/api/leave-decision.test.ts`; `__tests__/api/leave-cancel.test.ts`; `__tests__/api/leave-not-taken.test.ts` | DIRECT | DOMAIN_OR_LIFECYCLE_POLICY | None for covered domain invariants | `ALL` removes only a relationship restriction represented by the scope. |
...
| ADMIN-08 | Current ADMIN role | Employee/Admin route and transaction cases named in the test column | `DASHBOARD` | A stale route-time or client-supplied role must not remain authoritative after persisted role changes in the covered cases | Re-authenticate/re-read current role at the applicable boundary; non-ADMIN is denied | `modules/auth/application/sessions.ts`; Admin route auth; Employee transaction adapter | `__tests__/auth/auth-principal.test.ts`; `__tests__/lib/server-auth-token-version.test.ts`; `modules/employee/application/authorization.test.ts`; Admin mutation tests | INDIRECT | DOMAIN_OR_LIFECYCLE_POLICY | No mandatory Phase 11C.2 gap; `ACTOR-06` and `TX-06` directly prove the named cross-domain mutation cases, but this broader aggregate must not overclaim exhaustive ADMIN-route coverage | Current role comes from persistence, not the request; this aggregate remains INDIRECT. |
| COMPAT-01 | Employee compatibility | Employee read, stats, and export | `DASHBOARD` | Broad legacy read policy must remain explicit and must not be mistaken for narrow relationship policy | Current documented broad floor may allow the operation; no claim of migrated fine-grained PII policy is made | `modules/employee/application/authorization.ts` legacy read/stats/export mapping | `modules/employee/application/authorization.test.ts`; `__tests__/api/employees-routes.test.ts` | DIRECT | COMPATIBILITY_POLICY | No mandatory Phase 11C gap; revisit only through an explicit Employee broad-data policy decision | Employee PII/broad-data policy is outside Phase 11C and remains unchanged. |
| COMPAT-02 | Employee compatibility | Employee create/update/delete/import ADMIN floor | `DASHBOARD` | Legacy Admin floor must not be read as a central User grant or as an ADMIN lifecycle bypass | Explicit trusted ADMIN floor may authorize the adapter; lifecycle, validation, and transaction rules remain | `modules/employee/application/authorization.ts`; Employee mutation service | `modules/employee/application/authorization.test.ts`; `__tests__/api/employees-routes.test.ts`; `modules/employee/application/mutations.test.ts` | DIRECT | COMPATIBILITY_POLICY | No mandatory Phase 11C gap; keep the floor until a separately approved Employee migration replaces it | Exact structural/configuration denials do not bridge. |
...
| COMPAT-06 | Routine compatibility bridge | Routine work-item read with requested `scope=all` | `DASHBOARD` | The NO_APPLICABLE_GRANT to `ALL` bridge must apply only to the exact historical work-item case | Normal USER, `routine.task.read`, exact resolver denial reason, work-item view, and requested all may bridge; all other denials remain denials | `modules/routine/application/authorization.ts`; task occurrence route/service | `modules/routine/application/authorization.test.ts`; `__tests__/api/routines-occurrences.test.ts`; `__tests__/api/routine-summary.test.ts` for contrast | DIRECT | COMPATIBILITY_POLICY | No mandatory Phase 11C gap; do not retire or broaden this bridge; retirement requires a separately approved policy migration | Explicit grants, structural errors, unsupported channels, and non-work-item operations do not bridge. |
...
are not target-policy migration work in this route-coverage slice:
...
## Phase 11C.2 mandatory regression backlog — final status

The six mandatory Phase 11C.2 work items below are retained as the closure
record. All are complete; the final mandatory backlog is `0`. This record does
not include future policy selection and does not authorize production behavior
changes.
...
These items are useful future hardening but are not mandatory Phase 11C
regression work:
...
The following four decision families are not Phase 11C regression work and must
not increase the matrix `MISSING` count:
...
4. Approve any future Team policy and domain resource semantics. Team is not
   Department; no Team origin may be inferred from Department or Employee
   metadata; an explicit Team origin is required; no nested Teams or TeamRole
   inheritance may be introduced; and TeamRole names have no intrinsic
...
- The final Phase 11C.2D audit did not query or inventory the production
  authorization database; empty code-owned seed configuration is not evidence
  of empty persisted state. It is not proof that production has no Team,
  TeamRole, TeamMembership, TeamCapabilityGrant, TeamRoleCapabilityGrant, or
  UserCapabilityGrant records.
...
### Phase 11C.2C.1 verification record

The Phase 11C.2C.1 route tests use controlled persistence and lower
domain/export seams while keeping the HTTP composition, trusted actor
construction, registry, resolver, and applicable Routine query/mutation
authorization boundaries real. They do not require the MySQL integration
database, so `npm.cmd run test:integration:mysql` was not run for this slice.
No timeout configuration was changed.
...
### Phase 11C.2D final/current verification record

This is the final verification record for the current source at audit baseline
`5669d79ca359701bc6a637079ca738575b731cf7`. No production or test logic,
schema, migration, seed, registry, compatibility policy, deferred surface, or
timeout configuration changed during this closure. The default unit suite was
green on its first run; no architecture-timeout rerun was required.

| Command | Observed result |
| --- | --- |
| `npm.cmd run test:run -- modules/authorization/application/resolver.test.ts modules/authorization/application/grant-validation.test.ts modules/authorization/registry.test.ts modules/authorization/application/administration.test.ts modules/authorization/application/administration-mutations.test.ts` | PASS, exit 0. 5 files and 75 tests passed. |
| `npm.cmd run test:run -- modules/routine/application/authorization.test.ts modules/stock/application/authorization.test.ts modules/leave/application/authorization.test.ts __tests__/auth/workforce-transaction.test.ts` | PASS, exit 0. 4 files and 62 tests passed. |
| `npm.cmd run test:run -- __tests__/api/phase-11c2c1-employee-routine-route-authorization.test.ts __tests__/api/phase-11c2c2-stock-leave-route-authorization.test.ts` | PASS, exit 0. 2 files and 21 tests passed. |
| `npm.cmd run architecture:check` | PASS, exit 0. `Architecture check passed: checked 1121 repository source file(s) for module boundaries.` |
| `npm.cmd run lint:strict` | PASS, exit 0. ESLint completed with `--max-warnings=0`. |
| `npm.cmd run typecheck` | PASS, exit 0. `tsc --noEmit` completed successfully. |
| `npm.cmd run test:run` | PASS, exit 0. First run: 316 test files and 2,786 tests passed. |
| `npm.cmd run test:integration:mysql` | FAIL, exit 1. Prisma migrations passed with 67 migrations and no pending work; 15 of 16 integration files passed and 103 of 104 tests passed. The sole known unrelated failure was `__tests__/integration/leave-quota-concurrency.integration.test.ts` / `creates one quota for concurrent non-overlapping requests with different keys`, raising `WorkforceAuthorizationError` from `modules/leave/application/authorization.ts:parseUserRole` because of the malformed historical route mock role shape. |
| `git diff --check` | PASS, exit 0. No whitespace errors reported. |

## Production policy change statement

Production authorization policy was **not changed** in Phase 11C.2A, Phase
11C.2B, Phase 11C.2C.1, Phase 11C.2C.2, or Phase 11C.2D. The Phase 11C.2C.2
slice added only regression tests and matrix updates; Phase 11C.2D adds only
the final documentation reconciliation and closure record.
