...
canonical-only. The operator confirmed the production collision preflight and
migration deployment PASSED; the production transition is CLOSED. See
[the recipient migration runbook](./notification-capability-recipient-migration.md)
for the retained historical pre-cutover procedure.
