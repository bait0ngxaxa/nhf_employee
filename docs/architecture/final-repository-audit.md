...
| scripts/stock-inventory-audit.ts and MySQL test runner | Read-only Stock integrity audit and integration harness | tooling/application boundary | Audit reads a RepeatableRead snapshot and classifies canonical defaults; harness owns real-MySQL test setup, neither is runtime business ownership | A | Keep the permanent audit outside modules; no Stock backfill/repair command remains |
