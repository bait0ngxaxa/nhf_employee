...
| Routine task updates/deletes and occurrences | Version/state/conditional updates | Provider work is Outbox based for notifications. |
...
| Routine dashboard: task create/update/delete and occurrence admin edits under `app/api/routines/*`; Routine LIFF task create/update/delete under `app/api/line/routine/*` | Policies exist for `routine-task-create` 60, `routine-task-update` 120, `routine-task-delete` 60, and `routine-occurrence-admin` 180, but no current production Routine route calls the pre-auth function | `routine-task-create` 20; `routine-task-update` 40; `routine-task-delete` 20; `routine-occurrence-admin` 60 | Current Routine production callers consume only the authenticated user scope after server authentication and before the business handler. Task idempotency and occurrence state/version/authorization remain owned by Routine. The unused pre-auth policies are inventory/configuration, not evidence of active protection. |
