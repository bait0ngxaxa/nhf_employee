...
| `UserContext` from `lib/auth/context.ts` | Defined and constructed only through `lib/auth/api.ts`; its `auth.user` value is passed by the route groups above into capability services. Separate duplicate `UserContext` interfaces remain in `lib/services/audit-log/types.ts` and `lib/services/email-request/types.ts` | The narrow context is an application input, but duplicate service types and direct `auth.user` plumbing are compatibility debt to consolidate only after J1 seams are proven. |
