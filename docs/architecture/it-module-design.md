...
10. IT9A approves requester-only LIFF access. Remaining LIFF UI, shell integration, Ticket LINE notification, and device acceptance work is sequenced in IT9B–IT9E.
...
| IT9B — LIFF Self-Service UI + Conversation + Attachments | Add the requester-facing LIFF experience over the IT9A APIs. | LIFF presentation only; preserve requester-only policy and shared Ticket/application contracts. | UI and device-flow checks, including conversation and private attachments. |
| IT9C — LIFF Home / Navigation / Deep Link / Rich Menu Integration | Add IT to LIFF shell entry points and connect notification-ready Ticket destinations. | Home modules, bottom navigation, deep links, and Rich Menu changes only after the UI route exists. | Route, shell navigation, deep-link, and Rich Menu contract checks. |
| IT9D — IT Ticket LINE Notifications to LIFF | Add Ticket LINE delivery through the approved LIFF destination. | Reuse IT6 business event semantics and shared outbox; no duplicate Ticket event model. | Delivery, destination, retry/idempotency, privacy, and stale-recipient checks. |
| IT9E — IT Product E2E + Android/iPhone Acceptance | Validate the complete IT requester product on supported devices. | End-to-end requester flows and Android/iPhone acceptance. | Product E2E, responsive/accessibility, and device acceptance. |
| IT10 — Final Hardening + Compatibility + Retention Audit | Review deployed legacy rows, retention, operational failure modes and final ownership. | Only approved compatibility cleanup/migrations; explicit decision before any enum or grant change. | Architecture/security regression, relevant broad suite and deployment data review. |
...
IT9A adds no Prisma schema change or migration, `/liff/it` UI, Home/navigation/Rich Menu integration, operator/analytics LIFF route, or Ticket LINE/Email delivery. IT9B–IT9E and IT10 remain open.
