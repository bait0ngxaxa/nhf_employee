...
Status: **IT0 design contract; documentation only.** The decisions marked LOCKED define the intended boundary for later phases. No IT runtime, schema, grant, route, UI, or notification producer exists as a result of IT0. Product questions marked OPEN must be answered before the slice that depends on them. Current implementation wins over older phase documents.
...
| [`modules/authorization/contracts.ts`](../../modules/authorization/contracts.ts), [`registry.ts`](../../modules/authorization/registry.ts), [`application/resolver.ts`](../../modules/authorization/application/resolver.ts), [`composition.ts`](../../modules/authorization/application/composition.ts), [`evaluator.ts`](../../modules/authorization/application/evaluator.ts) | Actor carries trusted `userId`, optional `employeeId`, `systemRole`, channel; resolver evaluates configured Team, TeamRole, User grants. Domain default scopes are composed separately. `systemRole` is not business authority. Registry lacks domain `it` today. |
...
`WAITING_REQUESTER` is an active unresolved state, entered only when a requester response is needed; it is not a notification delivery state. Store entering/leaving timestamps as events so accumulated wait time survives multiple cycles. A status filter cannot manufacture authority. Assignment starts null (`UNASSIGNED`); `ASSIGNED` means a non-null User reference, whose current eligibility may later change. Only manage ALL may assign/reassign/unassign. Validate assignee is active/eligible at assignment time, independently of Team name or Department. Whether exactly one assignee suffices is OPEN; design one nullable assignee for the first slice. When an assignee later becomes inactive, retain the historical User reference and require a visible reassignment/unassignment path; do not silently transfer work.
...
| --- | --- | --- | --- |
| IT1 — Module foundation + authorization | Create proportional `modules/it` server/browser contracts, register five IT capabilities, define role-neutral defaults and operator grant setup. No Ticket mutation yet. | Registry/domain extension; trusted workforce actor, resource-policy interface and capability projection; no Email Request migration. | Focused resolver/default/channel/grant tests, architecture, lint/typecheck. |
| IT2 — Ticket persistence + workflow | Add Ticket/category/event and creation/transition/assignment domain operations, conditional concurrency, creation idempotency. Close workflow questions first. | Prisma migration, IT repository/service and server command contracts; no broad UI. | Focused domain + MySQL transaction/authorization/idempotency tests and migration validation. |
...
| IT6 — Notifications | Introduce approved Ticket events/channels. | IT semantic producer and public outbox dispatch seam; preserve legacy enum/history; Notification/Audit public calls. | Transaction/dedupe/recipient/retry/channel compatibility tests. |
