...
Status: **IT0 CLOSED; IT1 authorization foundation CLOSED; IT2 Ticket persistence and workflow CLOSED; IT3 user self-service CLOSED.** The `modules/it` server boundary owns the five Dashboard-only capabilities, Ticket persistence and workflow, requester-only queries, internal self-service API, and capability-projected Dashboard surface. Operator queue, comments/attachments, notifications, analytics, and Email Request migration remain deferred. Product questions marked OPEN must be answered before the slice that depends on them. Current implementation wins over older phase documents.
