...
Status: **IT0 CLOSED; IT1 authorization foundation CLOSED; IT2 Ticket persistence and workflow CLOSED; IT3 user self-service CLOSED; IT4 operator processing CLOSED.** The `modules/it` server boundary owns the five Dashboard-only capabilities, Ticket persistence and workflow, separate requester-only and read-ALL operator queries, internal APIs, and capability-projected Dashboard surfaces. Comments/attachments, notifications, analytics, and Email Request migration remain deferred. Product questions marked OPEN must be answered before the slice that depends on them. Current implementation wins over older phase documents.
...
IT4 adds no conversation/comments, internal notes, timeline UI, attachments, notifications, Audit producer, SLA/priority, analytics, cancel/close/reopen transitions, automatic resume/assignment, category administration, production Team/grant seeding, LIFF behavior, or Email Request migration. These remain deferred to IT5 or later phases; unresolved product policies remain open.
